    ...
    <nav class="navbar p-2 border-bottom">
      <div class="d-inline-block">
        <a class="navbar-brand" href="/">
          <img src="../public/logo.svg" alt="Logo" width="30" height="24" class="d-inline-block align-text-top">
      ...
      <ul class="d-inline-block">
        <BaseNavLi>
          <router-link to="/quienes-somos" class="text-decoration-none text-black link-primary">Acerca de
            nosotros</router-link>
        </BaseNavLi>
        <BaseNavLi>
          <router-link to="/servicios" class="text-decoration-none text-black link-primary">Servicios</router-link>
        </BaseNavLi>

        <template v-if="loggedUser.rol === 'admin'">
          <BaseNavLi><router-link to="/panel" class="text-decoration-none text-black link-primary">Panel
        ...
        <template v-if="user.id === null">
          <BaseNavLi><router-link to="/iniciar-sesion" class="btn btn-outline-primary">Iniciar sesion</router-link>
          </BaseNavLi>
          <BaseNavLi><router-link to="/registro" class="btn btn-sm btn-outline-secondary">Registro</router-link>
          </BaseNavLi>
        </template>
        <template v-else>
          <BaseNavLi>
          ...
          <BaseNavLi>
            <form action="#" @submit.prevent="handleLogout">
              <button class="btn btn-dark"><strong>{{ user.email }}</strong> Cerrar Sesion</button>
            </form>
          </BaseNavLi>
        </template>
      </ul>
